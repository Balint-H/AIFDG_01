...
class JCS:
    ...
    def get_remainder_axes(self):
        ...
        self.remainder_axes = \
            tuple(
                [tuple({0, 1, 2} - set(used_axes))[0] for used_axes in zip(self.body_fixed_axes, self.reference_axes)])
...
class MarkerSet(ABC):
    dict_mocap: Dict[str, np.ndarray]
    ...
    def se_load_mocap(self, load_emg=True):
        with C3DServer() as c3d:
            c3d.open_c3d(self.mocap_file)
            self.dict_mocap = c3d.get_marker_dict(self.list_marker)
            self.mocap_freq = c3d.get_video_frame_rate()
            if load_emg:
                self.dict_emg, self.emg_freq = self.get_emg_data(c3d)

    def save_json(self, save_path, indent=None, **kwargs):
        raise NotImplementedError('No default JSON dumping method!')
...
class KinematicSet(MarkerSet, ABC):
    dict_joint: Dict[str, Union[JCS, tuple]]
    dict_segment: Dict[str, Union[Segment, Callable]]

    def __init__(self, mocap_file, emg_file=None):
        super(KinematicSet, self).__init__(mocap_file=mocap_file, emg_file=emg_file)
...
class LegSet(KinematicSet):
    def __init__(self, mocap_file, emg_file=None):
        super(LegSet, self).__init__(mocap_file=mocap_file, emg_file=emg_file)
...
def worker_kinematic(args_in, set_class, save_dir_path, verbose=True, **kwargs):
    if verbose:
        print(multiprocessing.current_process().name + ' working on ' + args_in[0])
    ms: KinematicSet = set_class(*args_in, **kwargs)
    ms.se_proc_joints()
...
def parallel_proc(set_class: type, c3d_files, emg_files=None, save_dir_path='.', work_fun=worker_kinematic, n_pool=None,
                  **kwargs):
    ...
    if n_pool is None:
        n_pool = multiprocessing.cpu_count()

    w = partial(work_fun, set_class=set_class, save_dir_path=save_dir_path, **kwargs)
    import time
